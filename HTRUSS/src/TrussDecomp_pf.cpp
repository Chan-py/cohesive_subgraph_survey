...
int main(int argc, char *argv[]) {
  ...
  // PushForward enumeration (src/PushForward.cpp) instead of the O(worklist^2)
  // all-pairs Baseline: for edge a, reach each candidate third edge c DIRECTLY
  // through the nodes of a worklist edge b (c in V[v], v in E[b]) with a per-b
  // timestamp dedup, skipping the pair-tests on non-triangles. It enumerates the
  // SAME hyper-triangle multiset (same {a>b>c}, same source-disjoint + shared-node
  // conditions), so htn/adj — and hence trussness — are identical; only the
  // traversal is cheaper. Parallel over `a` with thread-local accumulation +
  // additive merge (integer sums are order-independent => bit-identical).
  #pragma omp parallel reduction(+:triCount)
  {
    vector<vector<vid_t>> source(Ecur);
    vector<eid_t> worklist;
    vector<eid_t> timestamp(Ecur, 0);
    eid_t stamp = 0;
    ...
    for (eid_t a = 0; a < Ecur; a++) {
      ...
      const long long wa = (long long)Gr.E[a].weight;
      const long long ra = repOf[a];
      for (const auto &b : worklist) {
        stamp++;
        const long long wb = (long long)Gr.E[b].weight;
        const long long rb = repOf[b];
        for (const auto &v : Gr.E[b]) {
          for (const auto &c : Gr.V[v]) {
            if (c >= b) break;                       // enforce c < b < a (each triple once)
            if (source[c].empty() || timestamp[c] == stamp) continue;  // c must touch a; dedup per b
            timestamp[c] = stamp;
            if (!vectorIntersect(source[b], source[c])) {  // a-b and a-c shared nodes disjoint
              ++triCount;
              const long long wc = (long long)Gr.E[c].weight;
              const long long rc = repOf[c];
              htn_l[ra] += wb * wc;
              htn_l[rb] += wa * wc;
              htn_l[rc] += wa * wb;
